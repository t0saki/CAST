...
async def concurrent_votes(
    ...
    for _ in range(num_requests):
        ...
        tasks.append(measure_request_time(client.vote, [username], [vote_count], ticket))
    ...
    for attempt in range(1, max_attempts + 1):
        try:
            ...
            if result.get('success', False):
                ...
                print(f"Vote failed after {successful_votes} successful attempts: {result.get('message')}")
                break
        except Exception as e:
            print(f"Exception after {successful_votes} successful attempts: {str(e)}")
            break
...
async def run_performance_tests(base_url: str, concurrency: int = 10):
    ...
    try:
        ...
        print(f"Successful requests: {concurrency_results['successful_requests']}")
        print(f"Failed requests: {concurrency_results['failed_requests']}")
        print(f"Average response time: {concurrency_results['avg_response_time']:.4f} seconds")
        print(f"Min response time: {concurrency_results['min_response_time']:.4f} seconds")
        print(f"Max response time: {concurrency_results['max_response_time']:.4f} seconds")
        ...
        print(f"Successful votes before limit: {limit_results['successful_votes']}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Performance testing for Little Vote system")
    parser.add_argument("--url", default="http://localhost:8000/graphql", help="GraphQL endpoint URL")
